...
export const updateForm = async (req, res) => {
  ...
  try {
    const id = req.params.id;
    const form = await FormLock.findByIdAndUpdate({ _id: id }, req.body, {
      new: true,
    });
    if (!form) {
      res
        .status(404)
        .json({
          message: "No se encontró el formulario con el id especificado.",
        });
    }
    res.status(200).json(form);
  } catch (error) {
...
export const deleteform = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "¡Formulario se ha eliminado correctamente!" });
  } catch (error) {
    res.status(400).json({ message: error.message });
  }
};
